...
class ReportGraph2 extends StatefulWidget {
  final List<ReportValues> reports;
  const ReportGraph2(this.reports);
  @override
  _ReportGraph2State createState() => _ReportGraph2State();
}

class _ReportGraph2State extends State<ReportGraph2> {
  ...
  void _initLineDataFromReport() {
    ...
    for (var report in widget.reports) {
      if (report.set == 2) {
        //Add values
        for (var value in report.getValues()) {
          values.add(new Entry(x: counter, y: value));
          values2.add(new Entry(x: counter, y: value / 2));

          counter += 1;
        }
        //Add spaces
        for (int i = 0; i < 10; i++) {
          values.add(new Entry(x: counter, y: i.toDouble()));
          counter += 1;
        }
      }
    }
    // for (var value in widget.reports.getValues()) {
    //   values.add(new Entry(x: counter, y: value));
    //   counter += 1;
    // }
    ...
    LineDataSet set2 = new LineDataSet(values2, "DataSet 1");
    ...
    _controller.data = LineData.fromList(List()
      ..add(set1)
      ..add(set2));
